...
export function findTokenByHash(obj, query, parent) {
  for (var key in obj) {
    var value = obj[key];

    if (typeof value === 'object') {
      var found = findTokenByHash(value, query, value);
      if(found) return found;
    }

    if (value === query) {
      //console.log('property=' + key + ' value=' + value + ' parent=' + JSON.stringify(parent));
      return parent;
    }
  }
}
